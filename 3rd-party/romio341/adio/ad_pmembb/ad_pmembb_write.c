                              ...
                              int *error_code)
{
    ...
    *error_code = MPI_SUCCESS;
    ...
    FPRINTF(stdout, "[%d/%d] ADIOI_PMEMBB_WriteContig called on %s\n", myrank, nprocs,
            fd->filename);
    FPRINTF(stdout, "[%d/%d]    writing (buf = %p, loc = %lld, sz = %lld)\n", myrank, nprocs, buf,
            (long long) offset, (long long) datatype_size * (long long) count);
                               ...
                               int *error_code)
{
    ...
    *error_code = MPI_SUCCESS;
    ...
    MPI_Comm_size(fd->comm, &nprocs);
    MPI_Comm_rank(fd->comm, &myrank);
    FPRINTF(stdout, "[%d/%d] ADIOI_PMEMBB_WriteStrided called on %s\n", myrank, nprocs,
            fd->filename);
